...
def get_data(algorithm, env_index, number):
    reward1 = smooth(np.load('DRL-code-pytorch-main/3.Rainbow_DQN/data_train/{}_env_{}_number_{}_seed_0.npy'.format(algorithm, env_name[env_index], number)))
    reward2 = smooth(np.load('DRL-code-pytorch-main/3.Rainbow_DQN/data_train/{}_env_{}_number_{}_seed_10.npy'.format(algorithm, env_name[env_index], number)))
    reward3 = smooth(np.load('DRL-code-pytorch-main/3.Rainbow_DQN/data_train/{}_env_{}_number_{}_seed_100.npy'.format(algorithm, env_name[env_index], number)))
...
def drawing_CP(plt, algorithm, number, color, label):
    reward, length = get_data(algorithm=algorithm, env_index=0, number=number)
    reward = np.array(reward).flatten()
    steps = np.arange(len(reward))
    sns.lineplot(x=steps, y=reward, color=color, linestyle='-')
...
def drawing_LL(plt, algorithm, number, color, label):
    reward, length = get_data(algorithm=algorithm, env_index=1, number=number)
    reward = np.array(reward).flatten()
    steps = np.arange(len(reward))
    sns.lineplot(x=steps, y=reward, color=color, linestyle='-')
    # sns.tsplot(time=np.arange(len), data=reward, color=color, linestyle='-')  # color=darkorange dodgerblue
